...
class cleancoded_Redirect {
	...
	public function setup() {
		 ...
		 *   add_filter( 'cleancoded_redirect_only_on_404', '__return_true' );
		 */
		if ( apply_filters( 'cleancoded_redirect_only_on_404', false ) ) {
	...
	public function match_redirect( $requested_path ) {
		...
		$redirects = apply_filters( 'cleancoded_registered_redirects', $redirects, $requested_path );

		// Allow for case insensitive redirects
		$case_insensitive = apply_filters( 'cleancoded_case_insensitive_redirects', true );
		...
		foreach ( (array) $redirects as $redirect ) {
			...
			if ( $matched_path ) {
				...
				$sanitized_redirect_to = esc_url_raw( apply_filters( 'cleancoded_redirect_to', $redirect_to ) );
	...
	public function maybe_redirect() {
		...
		if ( is_admin() || ( apply_filters( 'cleancoded_redirect_only_on_404', false ) && ! is_404() ) ) {
			return;
		}

		// get requested path and add a / before it
		$requested_path = esc_url_raw( apply_filters( 'cleancoded_requested_path', $_SERVER['REQUEST_URI'] ) );
		...
		do_action(
			'cleancoded_do_redirect',
			$requested_path,
			$matched_redirect['redirect_to'],
			$matched_redirect['status_code']
		);

		if ( defined( 'PHPUNIT_cleancoded_TESTSUITE' ) && PHPUNIT_cleancoded_TESTSUITE ) {
			// Don't actually redirect if we are testing
			return;
		}

		header( 'X-redirect-url: true' );

		// if we have a valid status code, then redirect with it
		if ( in_array( $matched_redirect['status_code'], cleancoded_get_valid_status_codes(), true ) ) {
