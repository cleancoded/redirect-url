<?php
/**
 * Setup cleancoded WP CLI commands.
 *
 ...
 */
class cleancoded_WP_CLI extends WP_CLI_Command {
	...
	public function cli_list() {
		...
		$redirects = cleancoded_get_redirects( array( 'post_status' => 'any' ), true );
	...
	public function create( $args ) {
		...
		$ret = cleancoded_create_redirect( $from, $to, $status_code, $enable_regex, $post_status );
	...
	public function import_htaccess( $args, $assoc_args ) {
		...
		foreach ( $pieces as $piece ) {
			...
			$sanitized_redirect_from = cleancoded_sanitize_redirect_from( $from );
			$sanitized_redirect_to   = cleancoded_sanitize_redirect_to( $to );

			$id = cleancoded_create_redirect( $sanitized_redirect_from, $sanitized_redirect_to, $http_status );
			if ( is_wp_error( $id ) ) {
	 ...
	 * ## EXAMPLE
	 *
	 *     wp redirect-url import redirections.csv
	 *
	...
	public function import( $args, $assoc_args ) {
		...
		foreach ( $args as $file ) {
			$processed = cleancoded_import_file( $file, $mapping );
			if ( ! empty( $processed ) ) {
